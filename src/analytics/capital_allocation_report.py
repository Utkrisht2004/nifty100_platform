...
def run_capital_allocation():
    ...
    for cid, group in grouped:
        if len(group) < 2:
            latest_patterns[cid] = (
                group.iloc[-1]["allocation_pattern"]
                if len(group) > 0
    ...
    if not changes_df.empty:
        ...
        print(
            f"[✓] Documented {len(changes_df)} companies with shifting allocation strategies -> {changes_path}"
        )
    else:
        pd.DataFrame(
            columns=[
        ...
        print("[✓] Zero pattern shifts detected year-over-year.")

    # 2. Update cashflow_intelligence.xlsx
    if intel_path.exists():
        intel_df = pd.read_excel(intel_path)
        intel_df["capital_allocation_label"] = (
            intel_df["company_id"].map(latest_patterns).fillna("Balanced Allocators")
        )
