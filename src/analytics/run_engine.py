...
from src.analytics.ratios import (
    calc_net_profit_margin, calc_operating_profit_margin, 
    calc_return_on_equity, calc_return_on_capital_employed, 
    calc_debt_to_equity, calc_interest_coverage, calc_asset_turnover
)
from src.analytics.cagr import calculate_cagr
from src.analytics.cashflow_kpis import (
    calc_free_cash_flow, calc_cfo_quality_score, 
    calc_capex_intensity, calc_fcf_conversion, classify_capital_allocation
)
...
def main():
    ...
    for idx, row in df.iterrows():
        ...
        opm = calc_operating_profit_margin(row['operating_profit'], row['sales'], row['opm_percentage'], cid, yr)
        roe = calc_return_on_equity(row['net_profit'], row['equity_capital'], row['reserves'])
        roce = calc_return_on_capital_employed(
            row['operating_profit'], row['equity_capital'], row['reserves'], row['borrowings'], is_fin
        )
        ...
        if roce is not None and pd.notna(row['roce_percentage']):
            if abs(roce - row['roce_percentage']) > 5.0:
                logger.warning(f"ROCE Anomaly | {cid} ({yr}): Calc={roce:.2f}%, Source={row['roce_percentage']}%")
        
        # Leverage & Efficiency
        de_val, high_lev_flag = calc_debt_to_equity(row['borrowings'], row['equity_capital'], row['reserves'], is_fin)
        icr_val, icr_label, icr_warn = calc_interest_coverage(row['operating_profit'], row.get('other_income', 0), row['interest'])
        ...
        cfo_score, cfo_label = calc_cfo_quality_score(row['operating_activity'], row['net_profit'])
        capex_int, capex_label = calc_capex_intensity(row['investing_activity'], row['sales'])
        ...
        cap_pattern = classify_capital_allocation(
            row['operating_activity'], row['investing_activity'], row['financing_activity'], cfo_label
        )
    ...
    count = pd.read_sql_query("SELECT COUNT(*) as cnt FROM financial_ratios", conn).iloc[0]['cnt']
