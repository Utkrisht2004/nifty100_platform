...
def generate_mock_peer_groups(conn, base_dir):
    ...
    if not file_path.exists():
        ...
        df = pd.read_sql_query("SELECT company_id, broad_sector as peer_group_name FROM sectors", conn)
...
def compute_percentiles():
    ...
    print(f"[✓] Saved {count} percentile records across {groups} peer groups into SQLite.")
