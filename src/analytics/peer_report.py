...
def generate_peer_report():
    ...
    for sector, group in master_df.groupby("peer_group_name"):
        ...
        pct_cols = [
            idx for idx, col in enumerate(group.columns, 1) if "_pct_rank" in col
        ]
    ...
    wb.save(output_path)
    print(
        f"[✓] Successfully generated {output_path} with {len(wb.sheetnames)} peer group sheets."
    )
