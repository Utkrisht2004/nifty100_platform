...
def create_radar_chart(
    company_id, company_data, peer_avg_data, metrics, output_dir, is_standalone=False
):
    ...
    plt.title(
        f"{company_id} Financial Profile {title_suffix}", size=14, weight="bold", y=1.1
    )
...
def main():
        ...
        for metric_label in metrics:
            if metric_label == "Composite Score":
                ...
                if is_standalone:
                    ...
                    peer_values.append(
                        peer_df["composite_quality_score"].median()
    ...
    print(
        f"[✓] Successfully exported {generated_count} radar charts to reports/radar_charts/"
    )
