...
def calc_operating_profit_margin(
    ...
    if source_opm is not None and abs(calculated_opm - source_opm) > 1.0:
        logger.info(
            f"OPM Mismatch | {company_id} ({year}): Calc={calculated_opm:.2f}%, Source={source_opm}%"
        )
...
def calc_return_on_capital_employed(
    ebit, equity_capital, reserves, borrowings, is_financial=False
):
