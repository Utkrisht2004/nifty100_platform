...
def generate_pros_cons():
    ...
    for cid, group in grouped:
        ...
        pats = group['net_profit_cr'].dropna().tolist() if 'net_profit_cr' in group.columns else []
        revs = group['revenue_cr'].dropna().tolist() if 'revenue_cr' in group.columns else []
        ...
        if len(des) > 0 and latest.get('debt_to_equity', 1) == 0:
            results.append((cid, 'Pro', 'P3', "Debt-free balance sheet provides financial flexibility and eliminates interest burden.", 95))
            
        # P4: Revenue CAGR > 15% over 5 years
        rev_cagr = latest.get('revenue_cagr_5yr', 0)
        if pd.notna(rev_cagr) and rev_cagr > 15:
            results.append((cid, 'Pro', 'P4', f"Revenue growing at above 15% CAGR over 5 years reflects strong business momentum.", 85))
        ...
        if pd.notna(div_yield) and div_yield > 2 and latest.get('free_cash_flow_cr', -1) > 0:
            results.append((cid, 'Pro', 'P8', "Consistent dividend yield above 2% backed by positive free cash flow.", 85))
            
        # P11: Revenue CAGR < PAT CAGR (Operating Leverage)
        if pd.notna(rev_cagr) and pd.notna(pat_cagr) and pat_cagr > rev_cagr > 0:
            results.append((cid, 'Pro', 'P11', "Revenue growing slower than profits shows improving operating leverage and scale benefits.", 80))
        ...
        if pd.notna(de) and de > 2.0 and not is_fin:
            results.append((cid, 'Con', 'C1', f"Debt-to-equity ratio of {de:.2f} is elevated for a non-financial company and warrants monitoring.", 90))
            
        # C2: FCF negative for 3 consecutive years
        if len(fcfs) >= 3 and all(f < 0 for f in fcfs[-3:]):
            results.append((cid, 'Con', 'C2', "Free cash flow negative for 3 consecutive years raises concern about cash generation quality.", 85))
        ...
        if pd.notna(payout) and payout > 100:
            results.append((cid, 'Con', 'C7', "Dividend payout ratio above 100% means the company is paying dividends from reserves, which is unsustainable.", 90))
            
        # C8: D/E rising for 3 consecutive years
        if len(des) >= 4 and des[-1] > des[-2] > des[-3]:
            results.append((cid, 'Con', 'C8', "Rising debt-to-equity ratio over 3 years suggests increasing financial leverage risk.", 80))
        ...
        if pro_count == 0:
            results.append((cid, 'Pro', 'P_FB', "Company holds a strong market position within the Nifty 100 universe.", 65))
        if con_count == 0:
            results.append((cid, 'Con', 'C_FB', "Macro-economic headwinds may impact near-term growth trajectories.", 65))

    # Compile and filter
    out_df = pd.DataFrame(results, columns=['company_id', 'type', 'rule_id', 'text', 'confidence_pct'])
    ...
    print(f"[✓] Verification: {companies_with_pros}/{total_companies} have Pros | {companies_with_cons}/{total_companies} have Cons.")
