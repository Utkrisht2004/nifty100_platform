...
def run_parser():
    ...
    try:
        ...
        print(f"[*] Successfully loaded analysis.xlsx (Skipped {header_row_idx} title rows).")
    ...
    for index, row in df.iterrows():
        ...
        cid_col = next((c for c in df.columns if 'company' in c or 'id' in c or 'ticker' in c), None)
        company_id = row[cid_col] if cid_col and pd.notna(row[cid_col]) else f"UNKNOWN_{index}"
    ...
    if not parsed_df.empty:
        ...
        print("[!] Still extracted 0 metrics. The text format in the Excel cells might not contain 'X Years: Y%' patterns.")
        
    if not failures_df.empty:
        failures_csv_path = output_dir / 'parse_failures.csv'
        failures_df.to_csv(failures_csv_path, index=False)
        print(f"[✓] Logged {len(failures_df)} unparseable entries to {failures_csv_path}")
    ...
    if not parsed_df.empty and db_path.exists():
        ...
        if not ratios_df.empty:
            ...
            if validation_flags:
                ...
                print(f"[!] Flagged {len(flags_df)} anomalies with >5% divergence. Saved to {flags_path}")
            else:
