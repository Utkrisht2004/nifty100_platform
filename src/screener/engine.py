...
class ScreenerEngine:
    ...
    def apply_filters(self, df, filters=None):
        ...
        if "net_profit_min" in filters:
            filtered_df = filtered_df[
                filtered_df["net_profit"] >= filters["net_profit_min"]
            ]
        ...
        if "rev_cagr_5yr_min" in filters:
            filtered_df = filtered_df[
                filtered_df["revenue_cagr_5yr"] >= filters["rev_cagr_5yr_min"]
            ]
        ...
        if "eps_cagr_min" in filters:
            filtered_df = filtered_df[
                filtered_df["eps_cagr_5yr"] >= filters["eps_cagr_min"]
            ]
        ...
        if "div_yield_min" in filters:
            filtered_df = filtered_df[
                filtered_df["dividend_yield_pct"] >= filters["div_yield_min"]
            ]
        ...
        if "div_payout_max" in filters:
            if "dividend_payout_ratio_pct" in filtered_df.columns:
                filtered_df = filtered_df[
                    filtered_df["dividend_payout_ratio_pct"]
                    <= filters["div_payout_max"]
                ]

        if "market_cap_min" in filters:
            filtered_df = filtered_df[
                filtered_df["market_cap_crore"] >= filters["market_cap_min"]
            ]

        # 6. Efficiency
        if "asset_turnover_min" in filters:
            filtered_df = filtered_df[
                filtered_df["asset_turnover"] >= filters["asset_turnover_min"]
            ]

        if "sales_min" in filters:
            filtered_df = filtered_df[
                filtered_df["market_cap_crore"] >= filters["sales_min"]
            ]
