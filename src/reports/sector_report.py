...
def generate_sector_reports():
    ...
    import re
    conn.create_function("REGEXP", 2, lambda expr, item: re.compile(expr).search(item) is not None)
    ...
    header_style = ParagraphStyle(name='Header', fontSize=18, textColor=colors.whitesmoke, backColor=colors.darkblue, spaceAfter=15, alignment=1, leftIndent=5)
    ...
    for sector in sectors:
        ...
        pdf_path = output_dir / f"{str(sector).replace('/', '_').replace(' ', '_')}_report.pdf"
        ...
        # Add Median Row
        median_row = ['SECTOR MEDIAN'] + [f"{medians[m[1]]:.2f}" if pd.notna(medians[m[1]]) else "N/A" for m in metrics]
        ...
        for _, row in sector_data.sort_values('company_name').iterrows():
            ...
            for _, col_name in metrics:
                ...
                comp_row.append(f"{val:.2f}" if pd.notna(val) and isinstance(val, (int, float)) else "N/A")
