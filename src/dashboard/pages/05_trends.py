...
st.markdown(
    "Overlay multiple fundamental metrics to visualize 10-year structural business trends."
)
...
selected_search = st.selectbox(
    "Search for a company:", options=[""] + companies["search_key"].tolist()
)
    ...
    if not ratios.empty and not pl.empty:
        ...
        if selected_metrics:
            ...
            for i, metric_label in enumerate(selected_metrics):
                ...
                hover_text = df.apply(
                    lambda row: (
                        f"{row['year']}<br>Value: {row[col_name]:.2f}<br>YoY: {row[f'{col_name}_yoy']:.1f}%"
                        if pd.notna(row[f"{col_name}_yoy"])
                        else f"{row['year']}<br>Value: {row[col_name]:.2f}"
                    ),
                    axis=1,
                )
            ...
            with st.expander("View Raw Time-Series Data"):
                display_df = df[
                    ["year"] + [metric_options[m] for m in selected_metrics]
                ].copy()
                st.dataframe(
                    display_df.set_index("year").sort_index(ascending=False),
