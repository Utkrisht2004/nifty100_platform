...
from src.dashboard.utils.db import get_companies, get_sectors, get_db_connection

st.set_page_config(page_title="Capital Allocation", layout="wide")
st.title("Capital Allocation Map 🧩")
st.markdown(
    "Visualizing how the Nifty 100 deploys capital using fundamental algorithmic signatures."
)
...
def load_capital_data():
    ...
    def classify_allocation(row):
        ...
        if div > 40 and fcf > 0:
            return "Aggressive Dividend Payers"
        if de == 0 and fcf > 1000:
            return "Cash Hoarders (Debt-Free)"
        if gro > 15 and fcf < 0:
            return "Growth / Heavy Reinvestment"
        if de > 2 and fcf > 0:
            return "Leveraged (Paying down Debt)"
        if de > 2 and fcf <= 0:
            return "Distressed / Highly Leveraged"
        if roe > 20 and fcf > 0 and div <= 40:
            return "Efficient Compounders"
        if roe < 10 and fcf > 0:
            return "Value Traps / Stagnant"
...
st.dataframe(
    pattern_df[display_cols].sort_values("return_on_equity_pct", ascending=False),
    hide_index=True,
