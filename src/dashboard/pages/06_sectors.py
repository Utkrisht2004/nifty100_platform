...
sector_df['market_cap_crore'] = sector_df['market_cap_crore'].fillna(sector_df['market_cap_crore'].median() or 5000)

st.markdown("---")
st.subheader(f"{selected_sector} Landscape")
st.markdown("X-Axis: Revenue (Size) | Y-Axis: ROE (Quality) | Bubble Size: Market Cap | Color: Sub-Sector")
...
medians = sector_df.groupby('sub_sector')[['return_on_equity_pct', 'net_profit_margin_pct', 'debt_to_equity']].median().reset_index()
