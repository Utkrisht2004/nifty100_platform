...
selected_search = st.selectbox(
    "Search for a company (Ticker or Name):", options=search_options
)
...
if selected_search:
    ...
    with st.container(border=True):
        ...
        st.markdown(
            company_info.get(
                "description", "Fundamental data overview based on recent filings."
            )
        )
    ...
    if not ratios_df.empty:
        ...
        with chart_col2:
            ...
            fig2.update_layout(
                margin=dict(t=30, b=0, l=0, r=0), yaxis_title="Percentage (%)"
            )
