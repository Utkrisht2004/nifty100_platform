...
selected_search = st.selectbox(
    "Search for a company to view filings:",
    options=[""] + companies["search_key"].tolist(),
)
...
if selected_search:
    ...
    with col1:
        ...
        st.markdown(
            "BSE and NSE block automated downloads. Click below to fetch the official PDFs directly via web search operators."
        )

        for year in [2024, 2023, 2022]:
            # Creates a URL that automatically searches for the exact PDF
            query = urllib.parse.quote(
                f'"{company_name}" OR "{ticker}" "Annual Report" {year} filetype:pdf'
            )
    ...
    with col2:
        ...
        st.markdown(
            "View all historical annual reports, concall transcripts, and investor presentations."
        )

        screener_url = (
            f"https://www.screener.in/company/{ticker}/consolidated/#documents"
        )
