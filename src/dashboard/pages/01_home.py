...
from src.dashboard.utils.db import get_sectors, get_companies, get_db_connection
...
st.markdown(
    "Macro-level fundamentals and sector distribution for the Nifty 100 universe."
)
...
if df.empty:
    ...
    debt_free_count = (
        len(df[df["icr_label"] == "Debt Free"]) if "icr_label" in df.columns else 0
    )
