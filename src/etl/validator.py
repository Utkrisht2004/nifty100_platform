...
class DataValidator:
        ...
        for table in ["profitandloss.xlsx", "balancesheet.xlsx", "cashflow.xlsx"]:
            if table in self.dfs:
                ...
                if not df.empty and all(
                    c in df.columns for c in ["company_id", "year"]
                ):
    ...
    def _check_dq04_bs_balance(self):
        if "balancesheet.xlsx" in self.dfs:
            ...
            if not df.empty and all(
                c in df.columns
                for c in ["company_id", "year", "total_assets", "total_liabilities"]
            ):
                    ...
                    if pd.notna(assets) and pd.notna(liabs):
                        if assets == 0 or abs((assets - liabs) / assets) > 0.01:
                            self.log_failure(
                                ...
                                f"Mismatch: Assets={assets}, Liabs={liabs}",
                            )
    ...
    def _check_dq05_opm_cross_check(self):
        if "profitandloss.xlsx" in self.dfs:
            ...
            if not df.empty and all(c in df.columns for c in req_cols):
                for _, row in df.iterrows():
                    op, sales, opm = (
                        pd.to_numeric(row["operating_profit"], errors="coerce"),
                        pd.to_numeric(row["sales"], errors="coerce"),
                        pd.to_numeric(row["opm_percentage"], errors="coerce"),
                    )
                    if (
                        pd.notna(op)
                        and pd.notna(sales)
                        ...
                        if abs(calculated_opm - opm) > 1.0:
                            self.log_failure(
                                ...
                                row["year"],
                                "opm_percentage",
                                f"Calculated {calculated_opm:.1f} vs Source {opm}",
                            )
    ...
    def _check_dq06_positive_sales(self):
        if "profitandloss.xlsx" in self.dfs:
            df = self.dfs["profitandloss.xlsx"]
            if not df.empty and all(
                c in df.columns for c in ["company_id", "year", "sales"]
            ):
                for _, row in df.iterrows():
        ...
        for table in ["profitandloss.xlsx", "balancesheet.xlsx", "cashflow.xlsx"]:
            if table in self.dfs:
                ...
                if not df.empty and "year" in df.columns:
                    invalid_years = df[
                        ~df["year"].astype(str).str.match(r"^\d{4}-\d{2}$", na=False)
                    ]
        ...
        if "companies.xlsx" in self.dfs:
            ...
            if not df.empty and "id" in df.columns:
                ...
                for _, row in invalid_tickers.iterrows():
                    self.log_failure(
                        "DQ-08", "WARNING", row["id"], None, "id", "Unnormalized ticker"
                    )
    ...
    def _check_dq09_net_cash_check(self):
        if "cashflow.xlsx" in self.dfs:
            ...
            if not df.empty and all(c in df.columns for c in req_cols):
                for _, row in df.iterrows():
                    ncf, cfo, cfi, cff = [
                        pd.to_numeric(row[c], errors="coerce") for c in req_cols[2:]
                    ]
    ...
    def _check_dq10_non_negative_fixed_assets(self):
        if "balancesheet.xlsx" in self.dfs:
            df = self.dfs["balancesheet.xlsx"]
            if not df.empty and all(
                c in df.columns for c in ["company_id", "year", "fixed_assets"]
            ):
                for _, row in df.iterrows():
    ...
    def _check_dq11_tax_rate_range(self):
        if "profitandloss.xlsx" in self.dfs:
            df = self.dfs["profitandloss.xlsx"]
            if not df.empty and all(
                c in df.columns for c in ["company_id", "year", "tax_percentage"]
            ):
                for _, row in df.iterrows():
    ...
    def _check_dq12_dividend_payout_cap(self):
        if "profitandloss.xlsx" in self.dfs:
            df = self.dfs["profitandloss.xlsx"]
            if not df.empty and all(
                c in df.columns for c in ["company_id", "year", "dividend_payout"]
            ):
                for _, row in df.iterrows():
    ...
    def _check_dq13_url_validity(self):
        if "documents.xlsx" in self.dfs:
            ...
            if not df.empty and all(
                c in df.columns for c in ["company_id", "year", "annual_report"]
            ):
                for _, row in df.iterrows():
                        ...
                        try:
                            ...
                            if res.status_code >= 400:
                                self.log_failure(
                                    ...
                                    row["year"],
                                    "annual_report",
                                    f"URL returning {res.status_code}",
                                )
                        except:
                            self.log_failure(
                                "DQ-13",
    ...
    def _check_dq14_eps_sign_consistency(self):
        if "profitandloss.xlsx" in self.dfs:
            df = self.dfs["profitandloss.xlsx"]
            if not df.empty and all(
                c in df.columns for c in ["company_id", "year", "net_profit", "eps"]
            ):
                for _, row in df.iterrows():
                    np_val, eps = pd.to_numeric(
                        row["net_profit"], errors="coerce"
                    ), pd.to_numeric(row["eps"], errors="coerce")
                    if pd.notna(np_val) and pd.notna(eps):
                        if np_val > 0 and eps <= 0:
                            self.log_failure(
                                ...
                                row["year"],
                                "eps",
                                "EPS mismatch with positive net profit",
                            )
    ...
    def _check_dq15_bse_nse_balance(self):
        if "balancesheet.xlsx" in self.dfs:
            ...
            if not df.empty and all(
                ...
                for _, row in df.iterrows():
                    assets, liabs = pd.to_numeric(
                        row["total_assets"], errors="coerce"
                    ), pd.to_numeric(row["total_liabilities"], errors="coerce")
