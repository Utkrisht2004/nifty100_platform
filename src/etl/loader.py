...
CORE_FILES = {
    ...
    'analysis.xlsx': 'analysis', 'documents.xlsx': 'documents',
...
def clean_data_for_db(dfs):
    ...
    if 'companies.xlsx' in dfs and not dfs['companies.xlsx'].empty:
        dfs['companies.xlsx'] = dfs['companies.xlsx'].drop_duplicates(subset=['id'], keep='last')
    
    valid_companies = set(dfs['companies.xlsx']['id'].dropna()) if 'companies.xlsx' in dfs else set()
    ...
    annual_tables = ['profitandloss.xlsx', 'balancesheet.xlsx', 'cashflow.xlsx', 'market_cap.xlsx']
    ...
    if 'stock_prices.xlsx' in dfs and not dfs['stock_prices.xlsx'].empty:
        ...
        dfs['stock_prices.xlsx'] = df.drop_duplicates(subset=['company_id', 'date'], keep='last')
    ...
    if 'balancesheet.xlsx' in dfs and not dfs['balancesheet.xlsx'].empty:
        ...
        if 'fixed_assets' in df.columns:
            df['fixed_assets'] = df['fixed_assets'].apply(lambda x: 0 if pd.notna(x) and float(x) < 0 else x)
...
def main():
    ...
    load_order = [
        'companies.xlsx', 'sectors.xlsx', 'analysis.xlsx', 'prosandcons.xlsx', 
        'profitandloss.xlsx', 'balancesheet.xlsx', 'cashflow.xlsx', 
        'stock_prices.xlsx', 'market_cap.xlsx', 'documents.xlsx'
    ]
    ...
    for filename in load_order:
        if filename in dfs and not dfs[filename].empty:
            ...
            try:
                ...
                audit_log.append({"table": table_name, "rows_loaded": len(df), "status": "SUCCESS"})
                print(f"    -> Loaded {len(df)} rows into '{table_name}'")
            except Exception as e:
                audit_log.append({"table": table_name, "rows_loaded": 0, "status": f"FAILED: {e}"})
